...
function isNewsChannel(channel) {
  return channel?.type === discord_js.ChannelType.GuildNews;
}
...
function isNewsThreadChannel(channel) {
  return channel?.type === discord_js.ChannelType.GuildNewsThread;
}
__name(isNewsThreadChannel, "isNewsThreadChannel");
function isPublicThreadChannel(channel) {
  return channel?.type === discord_js.ChannelType.GuildPublicThread;
}
__name(isPublicThreadChannel, "isPublicThreadChannel");
function isPrivateThreadChannel(channel) {
  return channel?.type === discord_js.ChannelType.GuildPrivateThread;
}
...
function isNsfwChannel(channel) {
  ...
  switch (channel.type) {
    ...
    case discord_js.ChannelType.GuildDirectory:
      return false;
    case discord_js.ChannelType.GuildNews:
    case discord_js.ChannelType.GuildText:
    case discord_js.ChannelType.GuildForum:
      return channel.nsfw;
    case discord_js.ChannelType.GuildNewsThread:
    case discord_js.ChannelType.GuildPrivateThread:
    case discord_js.ChannelType.GuildPublicThread:
...
function isAnyInteractableInteraction(messageOrInteraction) {
  if (messageOrInteraction instanceof discord_js.BaseInteraction) {
...
function actionIsButtonOrMenu(action) {
  return action.type === discord_js.ComponentType.SelectMenu || action.type === discord_js.ComponentType.Button;
}
...
var _PaginatedMessage = class {
  constructor({
    ...
    for (const page of this.pages) {
      if (utilities.isFunction(page) || utilities.isObject(page)) {
  ...
  async setUpMessage(messageOrInteraction, targetUser) {
    ...
    page = utilities.isFunction(page) ? await page(this.index, this.pages, this) : page;
    ...
    if (this.pages.length > 1) {
      const messageComponents = await Promise.all(
        [...this.actions.values()].map(async (interaction) => {
          return isMessageButtonInteractionData(interaction) ? new discord_js.ButtonBuilder(interaction) : new discord_js.StringSelectMenuBuilder({
  ...
  setUpCollector(channel, targetUser) {
    if (this.pages.length > 1) {
      this.collector = new discord_js.InteractionCollector(targetUser.client, {
        filter: (interaction) => !utilities.isNullish(this.response) && interaction.isMessageComponent() && this.actions.has(interaction.customId),
        time: this.idle,
    ...
    if (interaction.user.id === targetUser.id) {
      ...
      if (actionIsButtonOrMenu(action)) {
        ...
        if (!this.stopPaginatedMessageCustomIds.includes(action.customId)) {
          ...
          const updateOptions = utilities.isFunction(messagePage) ? await messagePage(newIndex, this.pages, this) : messagePage;
          await safelyReplyToInteraction({
  ...
  applyFooter(message, index) {
    ...
    if (lastEmbed) {
      const jsonTemplateEmbed = discord_js.isJSONEncodable(this.template.embeds?.[idx] ?? this.template.embeds?.[0]) ? (this.template.embeds?.[idx] ?? this.template.embeds?.[0])?.toJSON() : this.template.embeds?.[idx] ?? this.template.embeds?.[0];
  ...
  static resolveTemplate(template) {
    ...
    if (template instanceof discord_js.EmbedBuilder) {
...
__publicField(PaginatedMessage, "defaultActions", [
  {
    ...
    type: discord_js.ComponentType.SelectMenu,
...
var PaginatedFieldMessageEmbed = class extends PaginatedMessage {
  constructor() {
    super(...arguments);
    __publicField(this, "embedTemplate", new discord_js.EmbedBuilder());
  ...
  generatePages() {
    const template = this.embedTemplate instanceof discord_js.EmbedBuilder ? this.embedTemplate.toJSON() : this.embedTemplate;
  ...
  resolveTemplate(template) {
    ...
    if (utilities.isFunction(template)) {
      return template(new discord_js.EmbedBuilder());
    }
    return new discord_js.EmbedBuilder(template);
  }
};
